...
**/
	...
	tBTNodePtr parent;	//ukazatel na otce
	tBTNodePtr node;  	//pomocny ukazatel, inicializace na koren

	if(*RootPtr != NULL) {
		node = *RootPtr;

		do {						// FIND POSITION
			if((node->Cont) < Content)	{				// if Cont > 	=> right
				parent = node;
				node = node->RPtr;
			} else if ((node->Cont) > Content) {		// if Cont < 	=> left
				parent = node;
				node = node->LPtr;
			} else if ((node->Cont) == Content) {
				return;									// Duplicate so i do nothing
			}
		} while(node != NULL);

		node = malloc(sizeof(struct tBTNode));		// create new node
		if(node == NULL) return;

		node->LPtr = NULL;						 	// init new node
		node->RPtr = NULL;
		node->Cont = Content;
	...
	} else {										// Root is NULL => easy => create new node
...
**/
	...
	while(node != NULL ) {		// while left node != NULL
		SPushP(Stack, node);
		BTWorkOut(node);
		node = node->LPtr;		// move to next left node
	}
}
...
**/
	tStackP stack;				// onit stack
	SInitP(&stack);

	tBTNodePtr node = RootPtr;	// init node

	Leftmost_Preorder(node, &stack);
...
** realizujte jako volání funkce BTWorkOut().
**/
	tStackP stack;					// init stack
	SInitP(&stack);

	tBTNodePtr node = RootPtr;		// init node

	Leftmost_Inorder(node, &stack);
...
**/
	...
	tStackP stackP;  			// stackP & stackB
	tStackB stackB;

	SInitP(&stackP);			// init stacks
	SInitB(&stackB);
...
**/
	tStackP stackP;  								 	// stack with tBTNodePtr's
		...
		do {
			if((*RootPtr) != NULL)	{
				if((*RootPtr)->RPtr != NULL) {
					SPushP(&stackP, (*RootPtr)->RPtr);  // push
				}
				tBTNodePtr node;
				node = (*RootPtr);
				(*RootPtr) = (*RootPtr)->LPtr;  		// move to next left node
				free(node);
			} else {
				if(!SEmptyP(&stackP)) {
